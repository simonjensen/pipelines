...
- Keep `failCommentCondition: false` on `@semantic-release/github`. Without it, the plugin searches issues after every release (to close earlier failure issues) and opens an issue when a release fails. The job has no `issues` permission, so a published release fails with `Resource not accessible by integration` and the Docker step is skipped.
